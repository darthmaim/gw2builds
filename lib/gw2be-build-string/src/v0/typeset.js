...
export default {
    ...
    bitflags: jBinaryTemplate({
        setParams(bools) {
            this.baseType = bools.reduce((obj, cur) => ({ ...obj, [cur]: 'bool' }), {});
        },
        read() {
            const all = this.binary.read('bool');
            if (all) {
                return Object.keys(this.baseType).reduce((obj, cur) => ({ ...obj, [cur]: true }), {});
            }
            return this.baseRead();
        },
        write(data) {
            if (Object.values(data).every(v => v)) {
                return this.binary.write('bool', true);
            }
